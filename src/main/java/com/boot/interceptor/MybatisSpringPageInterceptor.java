...
public class MybatisSpringPageInterceptor implements Interceptor {
	...
	protected ThreadLocal<MybatisPage> pageThreadLocal = new ThreadLocal<MybatisPage>();
	...
	public Object intercept(Invocation invocation) throws Throwable {
		if (invocation.getTarget() instanceof StatementHandler) { // 控制SQL和查询总数的地方
			MybatisPage page = pageThreadLocal.get();
			if (page == null) { //不是分页查询
			...
			// 获取是否有分页Page对象
			MybatisPage<?> page = findPageObject(invocation.getArgs()[1]);
			if (page == null) {
				if (log.isTraceEnabled()) {
	...
	protected MybatisPage<?> findPageObject(Object parameterObj) {
		if (parameterObj instanceof MybatisPage<?>) {
			return (MybatisPage<?>) parameterObj;
		} else if (parameterObj instanceof Map) {
			for (Object val : ((Map<?, ?>) parameterObj).values()) {
				if (val instanceof MybatisPage<?>) {
					return (MybatisPage<?>) val;
				}
			}
		}
		return null;
	 ...
	 * @return
	 */
	protected String buildPageSql(MybatisPage<?> page, String sql) {
	 ...
	 * @return
	 */
	protected String buildMysqlPageSql(MybatisPage<?> page, String sql) {
	 ...
	 * @return
	 */
	protected String buildOraclePageSql(MybatisPage<?> page, String sql) {
	 ...
	 * @throws SQLException
	 */
	protected void queryTotalRecord(MybatisPage<?> page, Object parameterObject, MappedStatement mappedStatement, Connection connection) throws SQLException {
...
//		return "select count(*) " + sql.substring(index);
	...
	public static class PageNotSupportException extends RuntimeException {
		...
		private static final long serialVersionUID = 1873262540691756215L;
